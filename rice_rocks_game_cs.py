...
class Sprite:
    ...
    def collide(self, other):
        distance = dist(self.get_position(), other.get_position())
        has_collided = False
...
def draw(canvas):
    ...
    if started:
        if group_collide(rock_group, my_ship):
            lives -= 1
        hit_cnt = group_group_collide(missile_group, rock_group)
        score += hit_cnt
        if lives <= 0:
            started = False
            lives = 3
...
def rock_spawner():
    global rock_group, asteriod_info
    if started and (len(rock_group) < 12):
        rock_pos = [random.randrange(0, WIDTH), random.randrange(0, HEIGHT)]
        rock_vel = [random.random() * 1.6 - .3, random.random() * 1.6 - .3]
        rock_avel = random.random() * .2 - .1
        if dist(rock_pos, my_ship.get_position()) > my_ship.get_radius() + asteroid_info.get_radius() + 10:
